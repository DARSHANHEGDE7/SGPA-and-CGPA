...
public class GPACalculator1 {
	...
	public static void main(String[] args) {
		...
		while(flag)
		{
			...
			case 2:
				...
				System.out.println("enter the SGPA and Credit of each semester : ");
				System.out.println("SGPA\tCredit:");
				for (int i = 0; i < numberofsem; i++) {
					sgpaofeachsem[i]=scanner.nextDouble();
					 credit[i]=scanner.nextInt();
				}
				double cgpa=CgpaCalculator(sgpaofeachsem,credit,numberofsem);
	...
	private static double CgpaCalculator(double[] sgpaofeachsem,int credit[], int numberofsem) {
		double total=0;
		int totalcredit=0;

		for(int i=0;i<numberofsem;i++)
		{
			total=total+sgpaofeachsem[i]*credit[i];
